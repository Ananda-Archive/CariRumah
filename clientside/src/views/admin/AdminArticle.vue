            ...
            <div>
                <v-row justify="center">
                        ...
                        >
                            ...
                            <template v-slot:no-data>
                                <div class="mb-6 mt-md-10"  @click="createDialog = !createDialog">
                                    <v-icon style="font-size:100px">mdi-file-document</v-icon>
                                    <h3 class="mt-3">Belum Ada article, Ayo Tambahkan!</h3>
                                </div>
                            </template>
                        </v-data-table>
                    </v-col>
                    ...
                    <v-dialog v-model="createDialog" max-width="1100px" persistent>
                        <v-card>
                            <v-toolbar dense flat dark color="primary">
                                <span class="title font-weight-light">Tambah article</span>
                            ...
                            <v-form ref="form">
                                <v-card-text>
                                    <v-row>
                                        ...
                                        <v-col cols="12">
                                            <vue-editor :editorToolbar="document" placeholder="Deskripsi" v-model="article.content"></vue-editor>
                                        </v-col>
                                        <v-col cols="12">
                                            <v-file-input
                                                v-model="myFiles"
                    ...
                    <v-dialog v-model="editDialog" max-width="1100px" persistent>
                        <v-card>
                            <v-toolbar dense flat dark color="primary">
                                <span class="title font-weight-light">Edit article</span>
                                <v-btn absolute right icon @click="close"><v-icon>mdi-close</v-icon></v-btn>
                            </v-toolbar>
                            <v-form ref="form">
                                <v-card-text>
                                                ...
                                                <v-expansion-panel>
                                                    ...
                                                    <v-expansion-panel-content>
                                                        <v-row align="center">
                                                            <v-col cols="12">
                                                                <v-card>
                                                                    <v-img :src=article.image :lazy-src=article.image >
                ...
                <v-card-actions>
                    <v-container>
                        <v-row justify="center">
                            ...
                            <v-btn :disabled="loadingState" class="mt-n5" color="blue darken-1" text @click="deleteArticle">Ya</v-btn>
                        </v-row>
                    </v-container>
                </v-card-actions>
    ...
    methods: {
        ...
        addImage(file) {
            return new Promise((resolve, reject) => {
                var storageRef = firebase.storage().ref('article/'+Math.random() + '_'  + file.name)
                storageRef.put(file)
                .then((snapshot) => {
        ...
        async deleteArticle() {
            this.loadingRegister = true
            await this.deleteImage(this.article.image)
                .then((response) => {
